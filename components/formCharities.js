...
// Data
import dataForm from '../fakeData/formCharities.json'
import Button from './templates/button'

export default function FormCharities({ className }) {
  ...
  const handleSubmit = e => {
    ...
    let errors = []
      ...
      formData.append('charityname', e.target.charityname.value)
      formData.append('charityurl', e.target.charityurl.value)
      formData.append('contactname', e.target.contactname.value)
      formData.append('mobilephone', e.target.mobilephone.value)
      formData.append('email', e.target.email.value)
      formData.append('intent', e.target.intent.value)
      ...
      fetch(`${WP_API_URL}/contact-form-7/v1/contact-forms/90/feedback`, options)
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <form className={className} onSubmit={e=>handleSubmit(e)}>
          ...
          if(fi.type=="select") {
            return (
              <div key={i} className="mb-4">
                <select name={fi.name} defaultValue="" className="border-b border-[#cccccc] w-full outline-none">
                  <option value="" disabled>{fi.label}</option>
                  {
                    fi.options.map((op, ii)=>(
                      <option key={ii} value={op.value}>{op.label}</option>
                    ))
                  }
                </select>
      ...
      <div className="my-5">
        ...
        >
          { stateIsLoading ? `Sending, please wait... ` : `Submit form` }
        </Button>
      </div>
